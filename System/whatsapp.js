...
exports.serialize = (Styx, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && Styx.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(Styx.user && Styx.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        Styx.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        Styx.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => Styx.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? Styx.sendFile(chatId, text, "file", "", m, { ...options })
      : Styx.sendText(chatId, text, m, { ...options });
